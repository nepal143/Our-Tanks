...
namespace ChobiAssets.KTP
{
    ...
    public class Game_Controller_CS : MonoBehaviour
    {
        ...
        public List<ID_Control_CS> idScriptsList = new List<ID_Control_CS>(); // List of tank ID scripts.
        public List<Image> tankImages = new List<Image>(); // List of UI image
        public int currentID = 0;
        IEnumerator DelayedSwitchTank(float delay)
        {
            // Wait for the specified delay
            yield return new WaitForSeconds(delay);

            // Proceed to switch the current tank
            Switch_Tank();
        }
        void Switch_Tank_With_Delay(float delay)
        {
            // Start the coroutine with the specified delay
            StartCoroutine(DelayedSwitchTank(delay));
        }
        /*
       ...
       void Switch_Tank()
        {
            ...
            if (currentID >= idScriptsList.Count)
            {
                currentID = 0;
            }
            for (int i = 0; i < idScriptsList.Count; i++)
            ...
            // Disable all tank images.
            foreach (Image image in tankImages)
            {
                image.gameObject.SetActive(false);
            }

            // Enable the tank image corresponding to the current ID.
            tankImages[currentID].gameObject.SetActive(true);
        }
